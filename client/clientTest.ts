...
//   console.log("This is the PDA:", PDA);
...
console.log("object")
  ...
  await ataSetup(payer, mintKeypair, connection, TokenAccount, PDA);
  ...
  //     TokenAccount,
  //     payer.publicKey,
  //     10e9 * LAMPORTS_PER_SOL
  //   );
  // } catch (error) {
  //   console.log(error);
  // }

  // console.log("ATAaccountInfo, after minting: ", ATAAccountInfo);
}

async function vaultAccountSetup(connection: Connection) {
  const program = anchor.workspace
    .MemetokenLaunchpadSolana as Program<MemetokenLaunchpadSolana>;
  const [PDA, _] = PublicKey.findProgramAddressSync(
...
async function main() {
  ...
  const connection = new Connection(
    "https://weathered-attentive-star.solana-devnet.quiknode.pro/301196150b8590e1d266b64c9f640fa7298428ff/",
    "confirmed"
  );
  ...
  //   payer.publicKey
  // );
  // await vaultAccountSetup(connection);
  // await buy(payer, mintKeypair, connection, associatedTokenAccount, PDA);
  // await sell(payer, mintKeypair, associatedTokenAccount, PDA);
}
main();
